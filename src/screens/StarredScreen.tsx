...
export function StarredContent() {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
      <div style={{ padding: '18px var(--gutter-window) 12px', borderBottom: '1px solid var(--border-subtle)' }}>
      ...
      {starredFiles.length === 0 ? (
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px var(--gutter-window)' }}>
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        <Icon size={12} color="var(--text-brand)" />
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-brand)', letterSpacing: '0.04em' }}>
          {label}
        </span>
        <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
        <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{count}件</span>
      </div>
...
function StarredRow({
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 11, color: 'var(--text-muted)', marginRight: 14, fontFamily: 'var(--font-mono)' }}>
        {formatBytes(file.sizeBytes)}
      </span>
      <span style={{ fontSize: 11, color: recent ? 'var(--accent)' : 'var(--text-muted)', width: 80, textAlign: 'right' }}>
